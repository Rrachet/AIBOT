...
export async function GET(request: Request) {
  ...
  const { data, error } = await auth.supabase.from('campaigns').select('*').eq('workspace_id', auth.workspaceId).order('created_at', { ascending: false })
...
export async function POST(request: Request) {
  ...
  const { data, error } = await auth.supabase.from('campaigns').insert({ ...parsed.data, workspace_id: auth.workspaceId }).select('*').single()
